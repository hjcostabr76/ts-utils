...
import { NamingConventionCFG } from '@config/NamingConventionCFG'
...
export class ReflectionUtils {
    ...
    static getClassProp(className: string, propName: string): PropertyDeclaration {
        ...
        let prop: OrUndefinedTP<PropertyDeclaration> = classDeclaration.getProperty(propName)
    ...
    static getClassParent(nameOrDeclaration: string | ClassDeclaration): OrUndefinedTP<ClassDeclaration> {

        const isStringParam = (typeof nameOrDeclaration === 'string')
        let childClass: OrUndefinedTP<ClassDeclaration>
    ...
    static getInterfaceParents(nameOrDeclaration: string | InterfaceDeclaration): InterfaceDeclaration[] {
        const interfaceDeclaration = ReflectionUtils.getInterface(nameOrDeclaration)
        const parentNameRegex = new RegExp(`(.+.)?(${NamingConventionCFG.interface.prefix}\\w+)(<.+>)?$`)
